...
def helm_handler(event, context):
    ...
    values_file = None
    if not request_type == "Delete" and values_text is not None:
    ...
    if request_type == 'Create' or request_type == 'Update':
        ...
        if chart_asset_url is not None:
...
def get_oci_cmd(repository, version):
    ...
    private_ecr_pattern = r'oci://(?P<registry>\d+\.dkr\.ecr\.(?P<region>[a-z0-9\-]+)\.(?P<domain>[a-z0-9\.-]+))*'
    public_ecr_pattern = r'oci://(?P<registry>public\.ecr\.aws)*'
