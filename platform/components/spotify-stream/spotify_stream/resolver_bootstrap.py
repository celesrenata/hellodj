...
streaming task 2.3). ``boto3`` and ``hellodj_platform_logic.data_access`` /
...
(mirrors the tidal-stream ``resolver_bootstrap`` and the bot's
``guild_credentials.build_dynamo_credential_resolver``).
...
def build_user_credential_resolver(
    settings: SpotifyStreamSettings,
    ...
    the same non-fatal convention as the tidal-stream resolver bootstrap.
    ...
    try:
        ...
        log.info(
            "spotify-stream: unified per-user credential resolver wired "
        ...
        log.warning(
            "spotify-stream: unified credential resolver unavailable (%s) — "
            "per-user Spotify streaming is not ready", exc,
        )
        return None
