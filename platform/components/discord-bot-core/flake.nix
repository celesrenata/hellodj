  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachSystem [ "aarch64-linux" "x86_64-linux" ] (system:
      let
        ...
        # content. Without this, CI-generated bytecode (e.g. the gate step
        # importing hellodj_platform_logic before it is vendored in) lands in
        # the tree, and .pyc files embed timestamps — making the -src hash (and
        # the whole image closure) differ every run, so the S3 cache never hits.
        ...
        appSrc = pkgs.runCommand "hellodj-discord-bot-core-src" { src = filteredSrc; } ''
          mkdir -p $out/app
