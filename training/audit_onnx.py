"""Audit ONNX model: accuracy, recall, false positive rate on validation data."""
import sys
import os
import numpy as np
import onnx
...
ONNX_PATH = '/home/jovyan/Hello_DJ/Hello_DJ.onnx'
...
def load_fp_val_data():
    fp_val = np.load(FP_VAL_PATH)  # (481345, 96)
    # Reshape to match model input (16, 96) by sliding window
    n = INPUT_SHAPE[0]
...
def run_audit():
    ...
    # FP rate on large validation set (subsample for speed, batch size 1)
    n_fp_total = len(X_fp)
    n_fp_samples = min(50000, n_fp_total)
    X_fp_subset = X_fp[:n_fp_samples]
    fp_preds = []
    for i in range(len(X_fp_subset)):
        out = sess.run(None, {input_name: X_fp_subset[i:i+1]})[0]
        fp_preds.append(out[0, 0])
    fp_preds = np.array(fp_preds)
    fp_count = np.sum(fp_preds >= 0.5)

    val_set_hrs = 11.3  # matches training code
