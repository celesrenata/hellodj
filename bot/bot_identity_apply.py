"""HelloDJ — bot-side applier for per-guild bot identity (nickname + avatar).
...
Discord bot, it reads pending ``BOTIDENTITY`` items, diffs each against what has
already been applied, and applies only the changes — the server nickname via
``guild.me.edit(nick=...)`` (R2.7) and the per-guild server avatar via a raw
Discord REST ``PATCH /guilds/{guild_id}/members/@me`` (R2.8, Design Risk #2:
discord.py exposes no stable public method for the bot's OWN per-guild member
avatar). On success it writes ``apply_status="applied"`` + ``applied_at`` back to
the item and clears ``apply_error``; on ``discord.Forbidden`` (bot lacking the
Manage-Nicknames / guild permission) it records ``apply_status="error"`` + a
human-readable ``apply_error`` so the UI surfaces a clear error (R2.9).

Design for testability (fakes-friendly, matching the ``FakeSecrets`` style in
``playback/test_guild_credentials.py``):
...
* **Every side-effecting dependency is injected**: the DynamoDB-backed identity
  store (``IdentityStore`` protocol), the S3 client (``S3Reader`` protocol), the
  Discord client/guild lookup, and the raw-REST route seam. Task 7.3 wires a
  fake Discord + fake S3 + fake store; ``bot.py`` wires the real ones.

This module does NOT touch ``bot/cogs/video.py``'s global ``self.bot.user.avatar``
...
class IdentityApplier:
    ...
    ----------
    bot:
        The discord.py client (needs ``get_guild(id)`` and, for the avatar,
        ``http.request``). Injected so tests can pass a fake.
    store:
        The :class:`IdentityStore` reading desired identity + writing status.
    s3_client:
        A :class:`S3Reader` for the avatar bytes.
    avatar_bucket:
        The stage-scoped S3 bucket the web-ui uploaded avatar bytes to.
    route_factory:
    ...
    async def apply_guild(self, guild_id: str | int) -> ApplyOutcome:
        ...
        as ``apply_status="error"`` + a human-readable ``apply_error`` (R2.9).
        """
        gid = str(guild_id)
        ...
        if outcome.apply_nickname:
            ...
            except forbidden as exc:  # bot lacks Manage/Change Nickname (R2.9)
        ...
        if outcome.apply_avatar:
            ...
            except forbidden as exc:  # bot lacks the guild permission (R2.9)
                outcome.errors.append(
    ...
    async def apply_all_pending(self) -> dict[str, ApplyOutcome]:
        ...
        for guild in list(getattr(self._bot, "guilds", []) or []):
            ...
            except Exception:  # one guild must not abort the rest
    ...
    async def _apply_nickname(self, guild: Any, nickname: str) -> None:
        """Set the bot's server nickname via discord.py (R2.7)."""
    ...
    async def _apply_avatar(self, guild_id: str, avatar_key: str) -> None:
        """Set the bot's per-guild server avatar via a raw REST PATCH (R2.8).

        discord.py has no stable public method for the bot's OWN per-guild member
        avatar (Design Risk #2), so this issues the documented endpoint directly
        on the bot's already-authenticated HTTP session:
    ...
    def _write_back(self, guild_id: str, outcome: ApplyOutcome) -> None:
        ...
        combined human-readable message (R2.9) and the applied version is NOT
        advanced, so a corrected retry (or a permission fix) re-applies. On full
        success the version advances so the change is not re-applied.
        """
        if outcome.errors:
