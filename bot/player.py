...
async def _play_next_from_queue(guild_id: int, *, skip_history_push: bool = False) -> None:
    ...
    entry_url = next_entry.get("webpage_url") or next_entry.get("url") or next_entry.get("uri") or ""
...
async def _resolve_and_play(player: wavelink.Player, guild_id: int, entry: dict) -> None:
    ...
    try:
        ...
        # Sync PlaybackState to ws_hub so Activity clients can show audio progress
        try:
